...
struct Iterator
{
    ...
    Iterator& operator++()
    {
        ...
        if (ptr->next != nullptr)
        {
        //    double number = std::sqrt(ptr->data.GetCount());                 //ÂÐÅÌÅÍÍÛÉ ÒÈÏ
...
public:
    ...
    void AddHead(float digit) {
        ...
        else std::cout << "Ðàçìåð êîíòåéíåðà ïðåâûøåí" << std::endl;
    ...
    void Del(float digit) {
        ...
        if (x != nullptr) {
            if (x->next == nullptr) {
                ...
                while ((x->data == digit) && (x->next != nullptr)) {    //ÓÄÀËßÅÌ 1-É ÝËÅÌÅÍÒ
                    dx = x;
                    dx = dx->next;
                    x = nullptr;
                    x = dx;
                ...
                if (x != nullptr) {
                    ...
                    while (curr->next != nullptr) {     //ÓÄÀËßÅÌ ÑÅÐÅÄÈÍÍÛÅ
                        if (curr->data == digit) {
                            dx->next = curr->next;
                            curr = nullptr;
                            curr = dx->next;
                    ...
                    if (curr->data == digit) {             //ÓÄÀËÅÌ ÏÎÑËÅÄÍÈÅ
                        dx->next = nullptr;
                        curr = nullptr;
                        tail = dx;
                    }
...
public:
    ...
    Iterator<false> end() { return Iterator<false>(&mas[max_size]); }
    ...
    bool operator>(Array& other) {
        ...
        for (std::size_t i = 0; i < other.Size(); i++) {
            if (!(this->mas[i] > other.mas[i])) return false;
            else return true;
        }
    }
